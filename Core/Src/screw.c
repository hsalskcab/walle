...
extern uint16_t afeed_mm, move_sp, screw_step;
...
void screw_ext_left(){
    if(step==0){
        ...
        go_left(mm_min_to_clk(((double)(screw_step*rpm))/100, tickz_nm));
        step=2;
    }
...
void screw_ext_right(){
    if(step==0){
        ...
        go_right(mm_min_to_clk(((double)(screw_step*rpm))/100, tickz_nm));
        step=2;
    }
